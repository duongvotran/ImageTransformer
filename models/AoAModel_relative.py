...
from .TransformerModel_relative import LayerNorm, attention, attention_d, clones, SublayerConnection, PositionwiseFeedForward
...
class MultiHeadedDotAttention_d(nn.Module):
    ...
    def forward(self, query, value, key, flag, mask=None):
        ...
        # print(value_.shape)
        # print(mask.shape)
        x, self.attn = attention_d(query_, key_, value_, flag, mask=mask,
...
class MultiHeadedDotAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, scale=1, project_k_v=1, use_output_layer=1, do_aoa=0, norm_q=0, dropout_aoa=0.3):
        ...
        self.geo_linear = nn.Linear(64, 3)
    ...
    def forward(self, query, value, key, flag, mask=None):
        ...
        # print(value_.shape)
        # print(mask.shape)
        geo_fea = utils.BoxRelationalEmbedding(flag)
        # print(geo_fea.shape)
        geo_a = self.geo_linear(geo_fea)
        geo_attn = F.softmax(geo_a, dim=-1)
        x, self.attn = attention(query_, key_p, key_n, key_c, value_p, value_n, value_c, geo_attn, mask=mask,
                                 dropout=self.dropout)
        # print(x.shape)
...
class AoAModel_relative(AttModel):
    def __init__(self, opt):
        super(AoAModel_relative, self).__init__(opt)
        self.num_layers = 2
        # mean pooling
