...
class AttModel(CaptionModel):
    ...
    def clip_att(self, att_feats, att_masks, flag=None):
        ...
        if att_masks is not None:
            ...
            if flag is not None:
                flag = flag[:, :max_len, :max_len].contiguous()
                return att_feats, att_masks, flag
            else:
                return att_feats, att_masks

    def _prepare_feature(self, fc_feats, att_feats, att_masks, flag):
        att_feats, att_masks, flag = self.clip_att(att_feats, att_masks, flag)
        # import ipdb
        # ipdb.set_trace()
    ...
    def _forward(self, fc_feats, att_feats, flag_feats, seq, att_masks=None):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, flag_feats,
                                                                                   att_masks)
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state):
        ...
        xt = self.embed(it)
        self.core.img_info = self.img_info
    ...
    def _sample_beam(self, fc_feats, att_feats, flag_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            self.img_info = self.batch_info[k]
            for t in range(1):
                if t == 0:  # input <bos>
                ...
                logprobs, state = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats,
                                                          tmp_att_masks, state)
            # self.decode_step = self.core.attention.decode_step
    ...
    def _sample(self, fc_feats, att_feats, flag_feats, att_masks=None, opt={}):
        ...
        # import ipdb; ipdb.set_trace()
        self.batch_info = opt.get('batch_info', [])
        if beam_size > 1:
            return self._sample_beam(fc_feats, att_feats, flag_feats, att_masks, opt)

        batch_size = fc_feats.size(0)
        # assert batch_size==1, 'for saving data, comment it if batch size >1'
        # self.img_info = self.batch_info[0]
        state = self.init_hidden(batch_size)
