...
from .TransformerModel_b import LayerNorm, attention, attention_d, clones, SublayerConnection, PositionwiseFeedForward
# for decoder
...
class MultiHeadedDotAttention_d(nn.Module):
    ...
    def forward(self, query, value, key, mask=None):
        ...
        # print(value_.shape)
        # print(mask.shape)
        x, self.attn = attention_d(query_, key_, value_, mask=mask,
                                   dropout=self.dropout)

        
        # TODO: save the output of attention_d for analyze
        # self.img_info['id']
        # np.save('/home/liao/work_code/AoANet/data/tmp/attention_d_b/'+str(self.img_info['id'])+'.npy', x.data.cpu().numpy())
...
class MultiHeadedDotAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, scale=1, project_k_v=1, use_output_layer=1, do_aoa=0, norm_q=0, dropout_aoa=0.3):
        ...
        self.k_linears = nn.Linear(d_model, d_model * scale)  # (parent,neighbor,child)
        self.v_linears = nn.Linear(d_model, d_model * scale)
    ...
    def forward(self, query, value, key, flag, mask=None):
        ...
        if self.project_k_v == 0:
            ...
            key_ = self.k_linears(query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            value_ = self.v_linears(query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        ...
        # print(value_.shape)
        # print(mask.shape)
        x, self.attn = attention(query_, key_, value_, flag, mask=mask,
                                 dropout=self.dropout)
        # print(x.shape)
        # TODO: save the output of attention_d for analyze
        # import ipdb; ipdb.set_trace()
        # if self.att_layer_idx == 0:
        #     for i, img_info in enumerate(self.batch_info):
        #         # v = {'p_attn1': self.attn.data.cpu().numpy()[i], 'p_attn2': p_attn2.data.cpu().numpy()[i], 'p_attn3': p_attn3.data.cpu().numpy()[i]}
        #         with open('/home/liao/work_code/AoANet/data/tmp/attention_b_value_val/' + str(img_info['id']) + '.pkl', 'wb') as f:
        #             utils.pickle_dump(self.attn.data.cpu().numpy()[i], f)
...
class AoA_Refiner_Core(nn.Module):
    ...
    def forward(self, x, flag, mask):
        for i, layer in enumerate(self.layers):
            layer.att_layer_idx = i
            layer.batch_info = self.batch_info
            x = layer(x, flag, mask)
        return self.norm(x)
...
class AoA_Decoder_Core(nn.Module):
    ...
    def forward(self, xt, mean_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        self.attention.img_info = self.img_info
...
class AoAModel_b(AttModel):
    def __init__(self, opt):
        super(AoAModel_b, self).__init__(opt)
        self.num_layers = 2
        self.batch_info = []
        # mean pooling
    ...
    def _prepare_feature(self, fc_feats, att_feats, flag_feats, att_masks):
        ...
        self.refiner.batch_info = self.batch_info
