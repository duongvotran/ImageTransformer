...
# note sort the features batch according to the length of the att features
...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        # Note what is the embedding for??? No pretrained word embedding??
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        state = self.init_hidden(batch_size) # NOTE: what is it for

        outputs = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size + 1)

        # Prepare the features, i.e. encoder
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):  # Note the data should be copy beam_size times????
            ...
            for t in range(1):
                if t == 0:  # Note input <bos> start token?
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        temperature = opt.get('temperature', 1.0)  # Note what is temperature
        decoding_constraint = opt.get('decoding_constraint', 0)
        block_trigrams = opt.get('block_trigrams', 0)
        remove_bad_endings = opt.get('remove_bad_endings', 0)
        if beam_size > 1:
            return self._sample_beam(fc_feats, att_feats, att_masks, opt)
        # if not apply beam search
        batch_size = fc_feats.size(0)
